...
"""
import sys, statistics as st, random, datetime as dt
from lean_data import load_canonical_tape, Universe, CASH_TICKERS, TRANSFORM_VERSION
...
def windows(tape, uni, getreg, r0, pit=True, fill="NEXT_CLOSE"):
    ...
    for tk, s in tape.series.items():
        ...
        if len(s) < WIN + 10: continue
        full = [p for _, p in tape.series[tk]]
        fd   = [d for d, _ in tape.series[tk]]
        i0   = fd.index(s[0][0])
        dates = [d for d, _ in s]; px = [p for _, p in s]
        ...
        while i + WIN <= len(px):
            ...
            if (not pit) or (tk in uni.eligible_on(dates[i])):
                out.append({"tk": tk, "start": dates[i],
...
def score(res, key, base):
    ...
    wn  = sum(1 for x in e if x > .001); tie = sum(1 for x in e if abs(x) <= .001)
...
def block_boot(edges, reps=B_REPS, block=BLOCK, seed=SEED):
    ...
    rng = random.Random(seed); n = len(edges); meds = []
    nblocks = -(-n // block)
    for _ in range(reps):
        samp = []
    ...
    meds.sort()
    return meds[int(.025*reps)], meds[int(.5*reps)], meds[int(.975*reps)]
...
def cluster_boot(res, reps=B_REPS, seed=SEED+1):
    ...
    by = {}
    for w in res: by.setdefault(w["tk"], []).append(w["v2"] - w["v1"])
    tks = sorted(by); rng = random.Random(seed); meds = []
    for _ in range(reps):
        samp = []
        for _ in range(len(tks)): samp.extend(by[tks[rng.randrange(len(tks))]])
...
if __name__ == "__main__":
    ...
    for label, pit in (("CURRENT-ROSTER (no PIT)", False), ("POINT-IN-TIME", True)):
        ...
        w1 = score(res, "v1", "bh"); w2 = score(res, "v2", "bh"); h = score(res, "v2", "v1")
        print(f"{label:24s} windows {len(res):4d} | v1vsBH {w1[0]:3d}W/{w1[1]:2d}T/{w1[2]:3d}L "
