...
import d250r
from lean_data import NYSE_HOLIDAYS, Universe, is_session, load_canonical_tape
...
def _tape():
    return load_canonical_tape(TAPE)
...
def test_golden_d250r_headline():
    t = _tape()
    u = Universe.from_tape(t)
    g, r0 = d250r.regime_fn(t)
    res = d250r.windows(t, u, g, r0, pit=True)
    wins, tie, loss, med, _ = d250r.score(res, "v2", "v1")
    assert len(res) == 248
    assert (wins, tie, loss) == (150, 10, 88)
