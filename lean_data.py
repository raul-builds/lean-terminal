...
class Tape:
    ...
    def tickers(self): return sorted(self.series)
    def __len__(self):  return sum(len(v) for v in self.series.values())
...
def load_canonical_tape(path, drop_cash=False) -> Tape:
    ...
    for r in rows[1:]:
        try:
            d = dt.date.fromisoformat(r[di][:10]); c = float(r[ci])
        except (ValueError, IndexError):
            n_bad += 1; continue
        if d.weekday() >= 5:           n_weekend += 1; continue
        if d.isoformat() in NYSE_HOLIDAYS: n_holiday += 1; continue
        tk = r[ti].strip().upper()
        if drop_cash and tk in CASH_TICKERS: continue
    ...
    for tk, s in series.items():
        ...
        for i in range(1, len(s) - 1):
            ...
            if a and abs(b/a - 1) > V_SPIKE and abs(nx/b - 1) > V_SPIKE \
               and (b/a - 1) * (nx/b - 1) < 0:
                n_spike += 1; continue
            out.append(s[i])
        if len(s) > 1: out.append(s[-1])
        series[tk] = out
    return Tape(series, {"rows_kept": sum(len(v) for v in series.values()),
                         "weekend": n_weekend, "holiday": n_holiday,
                         "unparsed": n_bad, "v_spikes": n_spike,
                         "tickers": len(series)})
...
class Universe:
    ...
    def from_tape(cls, tape: Tape, cash=CASH_TICKERS):
        return cls({tk: (s[0][0], s[-1][0]) for tk, s in tape.series.items()
                    if tk not in cash})

    def eligible_on(self, day) -> set:
        if isinstance(day, str): day = dt.date.fromisoformat(day[:10])
