...
def run(price_path, holdings_path=None, retired=None):
    ...
    record("T02", "Required columns", "PASS" if not missing else "FAIL",
    ...
    bad = int(d.isna().sum())
    record("T04", "DATE parses", "PASS" if bad == 0 else "FAIL",
    ...
    other = len(s) - iso - us
    if iso and us:
        record("T05", "Single date format", "FAIL",
               f"D151 LIVE — {iso} ISO + {us} M/D/YYYY mixed in one column")
    elif other:
    ...
    nn = int(c.isna().sum())
    record("T06", "CLOSE numeric", "PASS" if nn == 0 else "FAIL",
           "all numeric" if nn == 0 else f"{nn} non-numeric/blank")
    df["CLOSEN"] = c

    # ---- T07 CLOSE positive
    nonpos = int((c <= 0).sum())
    record("T07", "CLOSE > 0", "PASS" if nonpos == 0 else "FAIL",
           "all positive" if nonpos == 0 else f"{nonpos} rows <= 0")

    # ---- T08 ticker hygiene
    t = df["TICKER"].astype(str)
    dirty = int((t != t.str.strip().str.upper()).sum())
    record("T08", "TICKER hygiene", "PASS" if dirty == 0 else "FAIL",
           f"{t.nunique()} tickers, clean" if dirty == 0
           else f"{dirty} rows with whitespace/case issues")

    # ---- T09 duplicate (DATE,TICKER)
    dup = int(df.duplicated(subset=["DT", "TICKER"]).sum())
    record("T09", "No duplicate DATE+TICKER", "PASS" if dup == 0 else "FAIL",
           "unique" if dup == 0 else f"{dup} duplicate pairs")

    # ---- T10 exact duplicate rows
    dupe_all = int(df.duplicated().sum())
    record("T10", "No exact duplicate rows", "PASS" if dupe_all == 0 else "FAIL",
           "none" if dupe_all == 0 else f"{dupe_all} identical rows")

    # ---- T11 no weekend rows
    wk = int(df["DT"].dt.dayofweek.isin([5, 6]).sum())
    record("T11", "No weekend rows", "PASS" if wk == 0 else "FAIL",
           "clean" if wk == 0 else f"{wk} Sat/Sun rows")

    # ---- T12 no future dates
    today = pd.Timestamp.today().normalize()
    fut = int((df["DT"] > today).sum())
    record("T12", "No future dates", "PASS" if fut == 0 else "FAIL",
    ...
    record("T13", "Coverage span", "PASS" if span >= 1.0 else "WARN",
           f"{sheet_min.date()} to {sheet_max.date()} ({span:.2f} yr, {len(cal)} sessions)")

    # ---- T14 sheet freshness
    behind = np.busday_count(sheet_max.date(), today.date())
    record("T14", "Sheet freshness", "PASS" if behind <= 1 else
           ("WARN" if behind <= 3 else "FAIL"),
    ...
    record("T15", "Sheet-wide session gaps", "PASS" if len(gaps) == 0 else "WARN",
           "none" if not gaps else
           f"{len(gaps)} missing weekdays (incl. holidays) e.g. "
           + ", ".join(str(g.date()) for g in gaps[:4]))

    # ---- T16 rows per ticker
    per = df.groupby("TICKER")["DT"].count()
    thin = per[per < len(cal) * 0.25]
    record("T16", "Rows per ticker", "PASS" if thin.empty else "WARN",
           f"median {int(per.median())} rows/ticker" if thin.empty
    ...
    record("T18", "D176 per-ticker staleness", "PASS" if not stale else "FAIL",
           f"all {len(fresh)} current" if not stale
    ...
    fmt = lambda d: ", ".join(f"{k} {v}d ({last[k].date()})" for k, v in sorted(d.items(), key=lambda x: -x[1]))
    msg = []
    if dead_real: msg.append(f"{len(dead_real)} DEAD (on roster, no writes >={DEAD_DAYS}d): " + fmt(dead_real))
    if dead_ret:  msg.append(f"{len(dead_ret)} RETIRED (off roster, expected): " + fmt(dead_ret))
    record("T19", "D176 dead feeds (roster-aware)",
    ...
    record("T21", "Extreme daily moves", "PASS" if ext.empty else "WARN",
           f"none >{EXTREME_MOVE:.0%}" if ext.empty
           else f"{len(ext)} moves >{EXTREME_MOVE:.0%}: "
                + ", ".join(f"{r.TICKER} {r.DT.date()} {r.RET:+.0%}"
                            for r in ext.head(5).itertuples()))
    ...
    record("T22", "Scale corruption", "PASS" if scale.empty else "FAIL",
           f"no close >{SCALE_RATIO}x off ticker median" if scale.empty
           else f"{len(scale)} rows: "
                + ", ".join(f"{r.TICKER} {r.DT.date()} ${r.CLOSEN:,.2f}"
                            for r in scale.head(5).itertuples()))
    ...
    record("T23", "Frozen price feeds", "PASS" if not frozen else "WARN",
           f"none (>={FROZEN_RUN} identical closes)" if not frozen
           else ", ".join(f"{k} {v} sessions flat" for k, v in frozen[:6]))

    # ---- T24 null closes by ticker
    nullc = df[df["CLOSEN"].isna()].groupby("TICKER").size()
    record("T24", "Null closes by ticker", "PASS" if nullc.empty else "FAIL",
           "none" if nullc.empty
           else ", ".join(f"{k} {v}" for k, v in nullc.items()))
    ...
    if "MOVE_%" in df.columns:
        ...
        if both.sum() > 0:
            ...
            record("T25", "MOVE_% consistency", "PASS" if badm == 0 else "WARN",
                   f"{both.sum():,} checked, agree" if badm == 0
    ...
    def benford(series, label, tid, group=None):
        ...
        if group is not None:
            ...
            if orders.median() < 1.0:
                record(tid, f"D171 Benford ({label})", "SKIP",
        ...
        digits = (series.dropna().astype(float).abs().astype(str)
        ...
        top = ", ".join(f"{k}:{obs.get(k,0)*100/n:.1f}%(exp {exp[k]*100/n:.1f}%)"
                        for k in ["1", "2", "3"])
    ...
    if len(cents) >= 500:
        ...
        record("T27", "Last-digit uniformity", st,
    ...
    if holdings_path:
        try:
            ...
            wcol = next((c for c in h.columns if "WEIGHT" in c), None)
            tcol = next((c for c in h.columns if "TICKER" in c or "SYMBOL" in c), None)
            shcol = next((c for c in h.columns if "SHARE" in c), None)

            if valcol and tcol:
                hv = pd.to_numeric(h[valcol].astype(str).str.replace(r"[$,]", "", regex=True),
                                   errors="coerce")
                total = hv.sum()
                w = hv / total if total else hv * 0
                over = h.loc[w > MAX_WEIGHT, tcol].tolist()
                record("T28", "D164 position weight", "PASS" if not over else "FAIL",
                       f"book ${total:,.0f}, max weight {w.max():.1%}" if not over
                       else f"{over} exceed {MAX_WEIGHT:.0%} of ${total:,.0f} book")

                bigger = h.loc[hv > total, tcol].tolist()
                record("T29", "D164 value vs book total", "PASS" if not bigger else "FAIL",
                       "no position exceeds book total" if not bigger
                       else f"{bigger} larger than entire book")
            else:
            ...
            if shcol and tcol:
                ...
                impl = [(h.loc[i, tcol], sh[i] * lastpx.get(h.loc[i, tcol], np.nan))
                        for i in h.index if pd.notna(sh.get(i))]
                bad = [(t, v) for t, v in impl if pd.notna(v) and v > total * 1.5]
                record("T30", "D164 shares x price sanity", "PASS" if not bad else "FAIL",
                       "implied values consistent" if not bad
        ...
        for tid, nm in (("T28", "D164 position weight"),
    ...
    record("T31", "Cash sleeve tagged", "PASS" if present else "WARN",
           f"{sorted(present)} present and flagged non-tradeable" if present
           else "no cash ticker found in PRICE_HISTORY")

    # ---- T32 encoding / stray whitespace
    stray = sum(int(df[c].astype(str).str.contains(r"^\s|\s$", regex=True, na=False).sum())
                for c in ["DATE", "TICKER", "CLOSE"])
    record("T32", "No stray whitespace", "PASS" if stray == 0 else "WARN",
...
if __name__ == "__main__":
    ...
    hold = None; ret = None
    for a in sys.argv[2:]:
        if a.upper().startswith("RETIRED="): ret = a.split("=",1)[1].split(",")
        else: hold = a
